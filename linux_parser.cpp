...
long LinuxParser::IdleJiffies() { 
  vector<string> jiffies = CpuUtilization();
  return  (long)std::stol(jiffies[3]) + (long)std::stol(jiffies[4]);
}
...
int LinuxParser::TotalProcesses() { 
  return std::stoi(LinuxParser::Parser("processes", kProcDirectory + kStatFilename).c_str());
}

// TODO: Read and return the number of running processes
int LinuxParser::RunningProcesses() {
  return std::stoi(LinuxParser::Parser("procs_running", kProcDirectory + kStatFilename).c_str());
}
